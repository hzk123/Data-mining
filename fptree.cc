...
{
	string path = "in.txt";
	vector<transaction> transactions = readFile(path);//argv[1]是输入文件的路径，输入文件格式参照根目录下transactions文件
	...
	string tmp="";
	generateFrequentItems(headerTable, fpTreeRoot, tmp, modeLists);//递归挖掘Fp-tree
